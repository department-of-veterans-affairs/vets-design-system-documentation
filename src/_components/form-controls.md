...
Conditional fields make forms more digestable and less confusing by revealing fields only when they become relevant based on a users response to a question.

<div class="site-showcase">
{% include_relative html/conditional-fields.html %}
</div>

{% include snippet.html content='html/conditional-fields.html' %}
...
- When additional information from the user is required based on how they respond to a question

#### When to consider something else

- When the information is required regardless of the users response to other questions in the form

#### Guidance

- Use conditional fields after inputs where the user is only allowed to choose one option, such as a radio or select element
- The conditional fields should appear immediately after the input that reveals them (the parent input)
- Visually relate the conditional fields to their parent by applying the blue left border pattern to the field grouping which includes the parent element label, input, as well as conditional field label and input (see example)