...
* **Context.** Status tags must have semantic context. Users must be able to easily identify which element the tag is associated with. For example, place the status tag in a heading that identifies what has that status.
