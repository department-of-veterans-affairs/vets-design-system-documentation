...
* **Context.**  Status Tags must have clear context in the page’s semantic structure and reading order. Users must be able to easily identify which element each tag is associated with. For example, place the Status Tag in a heading that identifies what has that status.
