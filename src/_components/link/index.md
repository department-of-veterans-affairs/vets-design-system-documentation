...
<div class="mermaid-width-wide">
  {% include mermaid-chart.html 
   ...
   chart="flowchart TD
    Start[How should this link open?]:::node-start --> Platform{What platform?}:::node-question
    ...
    %% Mobile App Flow  
    Platform --> Mobile[Mobile App]:::node-answer-secondary
    Mobile --> MobileType{What type of link?}:::node-question
    
    MobileType --> ContentLink[Content link<br/>Reading pages, viewing info]:::node-answer-primary
    ContentLink --> ContentLocation{Where is the content?}:::node-question
    
    ContentLocation --> InApp[Within the app]:::node-answer-primary
    InApp --> FullPanel[Open in FULL PANEL<br/>Stay within app]:::node-result-link
    
    ContentLocation --> OutApp[Outside the app]:::node-answer-secondary
    OutApp --> SignInNeeded{Requires sign-in or<br/>is third party?}:::node-question
    
    SignInNeeded --> SignYes[YES]:::node-answer-primary
    SignYes --> Browser[Open in BROWSER<br/>Show alert warning<br/>User leaves app]:::node-result-action
