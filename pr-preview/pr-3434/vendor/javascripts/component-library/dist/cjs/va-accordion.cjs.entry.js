...
const VaAccordion = class {
    constructor(hostRef) {
        ...
        this.expandCollapseAll = (expanded) => {
            ...
            utils.getSlottedNodes(this.el, 'va-accordion-item').forEach((item) => {
    ...
    itemToggledHandler(event) {
        ...
        if (this.openSingle) {
            utils.getSlottedNodes(this.el, 'va-accordion-item')
    ...
    render() {
        ...
        const accordionClass = index$1.classnames({
            'usa-accordion': true,
        });
        const accordionItemIDs = [...this.el.children]
            .filter((el) => el.tagName.toLowerCase() === 'va-accordion-item')
            .map((el) => el.id);
        return (index.h(index.Host, { key: 'cf227d8049ad6b7b5fb35d8ffd7c9b463ca11485' }, index.h("div", { key: 'a44e2be0b6f3076998fb17ea2c9f21f9cae3e55b', class: accordionClass, ref: (accordionContainer) => this.accordionContainer = accordionContainer }, !openSingle ? (index.h("button", { class: "va-accordion__button", ref: el => (this.expandCollapseBtn = el), onClick: () => this.expandCollapseAll(!this.expanded), "aria-label": this.expanded
                ? i18next.instance.t('collapse-all-aria-label')
                : i18next.instance.t('expand-all-aria-label'), "aria-controls": accordionItemIDs.join(' '), "aria-expanded": `${this.expanded}` }, this.expanded
            ? `${i18next.instance.t('collapse-all')} -`
            : `${i18next.instance.t('expand-all')} +`)) : null, index.h("slot", { key: '4c71a973c15cc8881be11ba84ddaa15326883958' }))));
    }
    get el() { return index.getElement(this); }
};
VaAccordion.style = VaAccordionStyle0;

exports.va_accordion = VaAccordion;
